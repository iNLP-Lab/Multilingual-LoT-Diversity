...
def parse_args():
    ...
    parser.add_argument("--data_name", type=str, choices=['novelty', 'infinity'])
...
path = "prompt_for_thinking_language_control.json"
...
def main(
    ...
    if args.data_name == "novelty":
        path = "data/novelty-bench"
        path = os.path.join(path, "curated")
        data = load_from_disk(path)
        questions = data['prompt']
    elif args.data_name == "infinity":
        ds = load_dataset("liweijiang/infinite-chats-eval")
        questions = ds['train']['query']
    ### Step 1. Thinking Language Control and Repeated Sampling (Sec 3.1)
    language = args.language
    language_system_prompt = lang_prompt_final_dict[language]['system_prompt']
    language_instruction = lang_prompt_final_dict[language]['instruction']
    ...
    sampling_params = SamplingParams(
            temperature=args.temperature, max_tokens=8192, n=args.sample_num, seed=42, top_p=0.95, top_k=20,
        )
    results = []
    ...
    output_path = f"{args.output_path}/{model.split('/')[-1]}_{language}_{args.temperature}_{args.sample_num}_{args.flag}_dp_rank_{global_dp_rank}.jsonl"
    with open(output_path, "w") as fout:
        for item in results:
            fout.write(json.dumps(item, ensure_ascii=False) + "\n")
    ### Step 2. Regenerate the English final output with Output Language Control (Sec 4.1)
    tokenizer = llm.get_tokenizer()
    
    new_output_dir = args.output_path + "_english_answer"
    os.makedirs(new_output_dir, exist_ok=True)
    new_prompts = []
    new_ids = []
    generation_ids = []
    for res_id, x in enumerate(results):
        idx = x['idx'] # question_idx
        generations = x['responses']
        old_prompt = x['prompt']
        question = questions[idx]
        for gen_id, generation in enumerate(generations):
            if '</think>' in generation and len(generation.split('</think>')) == 2:
                en_system_prompt = lang_prompt_final_dict['en']['system_prompt']
                en_instruction = lang_prompt_final_dict['en']['instruction']
                msgs = [{"role": "system", "content": en_system_prompt}, {"role": "user", "content": f"{en_instruction}\n\nQuestion: {question}\n"}]
                p1 = tokenizer.apply_chat_template(msgs,tokenize=False,add_generation_prompt=True,enable_thinking=True)
                p1 += f"<think>\n{language_prefix}"
                think, answer = generation.split("</think>")
                new_prompt = p1 + think + '</think>\n\nLet me provide my answer in English only:'
                new_prompts.append(new_prompt)
                new_ids.append(res_id)
                generation_ids.append(gen_id)
        else:
            continue
    
    sampling_params = SamplingParams(
        temperature=args.temperature, max_tokens=8192, n=1, seed=42, top_p=0.95, top_k=20,
    )
    outputs = llm.generate(new_prompts, sampling_params)
    for id_, output in enumerate(outputs):
        res_id = new_ids[id_]
        gen_id = generation_ids[id_]
        final_answer = [o.text for o in output.outputs][0] # sample 1
        if "gen_id" not in results[res_id]:
            results[res_id]["gen_id"] = []
        if "gen_prompt" not in results[res_id]:
            results[res_id]["gen_prompt"] = []
        if "final_answer" not in results[res_id]:
            results[res_id]["final_answer"] = []
        results[res_id]["gen_id"].append(gen_id)
        results[res_id]["gen_prompt"].append(output.prompt)
        results[res_id]["final_answer"].append(final_answer)

    # Save to file per DP rank
    new_output_path = f"{new_output_dir}/{model.split('/')[-1]}_{language}_{args.temperature}_{args.sample_num}_{args.flag}_dp_rank_{global_dp_rank}.jsonl"
